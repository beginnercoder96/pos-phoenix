...
CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE,email TEXT NOT NULL UNIQUE,display_name TEXT NOT NULL,password_hash TEXT NOT NULL,role TEXT NOT NULL CHECK(role IN ('superadmin','operator','barberman','cashier','manager')),active INTEGER NOT NULL DEFAULT 1,branch_id INTEGER REFERENCES branches(id),staff_type TEXT CHECK(staff_type IN ('barberman','cashier','manager','owner') OR staff_type IS NULL),phone_number TEXT,bank_name TEXT,bank_account_number TEXT,totp_secret TEXT,totp_enabled INTEGER NOT NULL DEFAULT 0,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
CREATE TABLE IF NOT EXISTS pre_auth_tokens (token_hash TEXT PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,failed_attempts INTEGER NOT NULL DEFAULT 0,locked_until DATETIME,expires_at DATETIME NOT NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP);
