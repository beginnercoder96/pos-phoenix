...
CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT NOT NULL UNIQUE,display_name TEXT NOT NULL,password_hash TEXT NOT NULL,role TEXT NOT NULL CHECK(role IN ('superadmin','operator','barberman','cashier','manager')),active INTEGER NOT NULL DEFAULT 1,branch_id INTEGER REFERENCES branches(id),staff_type TEXT CHECK(staff_type IN ('barberman','cashier','manager','owner') OR staff_type IS NULL),created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP);
